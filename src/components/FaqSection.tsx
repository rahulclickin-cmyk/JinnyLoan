...
export const FaqSection: React.FC<FaqSectionProps> = ({
  ...
  return (
    <section id="faq" className="py-6 sm:py-14 bg-white border-b border-slate-200">
      <div className="max-w-4xl mx-auto px-3 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-4 sm:mb-8">
          ...
          <p className="text-[11px] sm:text-sm text-slate-500 mt-1">
            Everything you need to know about loan eligibility, process & documents
          </p>
        </div>
            ...
            return (
              <div
                ...
                className={`border rounded-xl sm:rounded-2xl overflow-hidden transition-all ${
                  isOpen 
                    ? 'border-[#1e3a8a]/30 bg-blue-50/20 shadow-xs' 
              ...
              >
                <button
                  ...
                  className="w-full text-left py-2.5 px-3.5 sm:py-4 sm:px-6 flex items-center justify-between gap-2.5 font-bold text-slate-900 text-xs sm:text-base focus:outline-none cursor-pointer"
                  aria-expanded={isOpen}
                >
                  <span className={`${isOpen ? 'text-[#1e3a8a]' : 'text-slate-800'} text-xs sm:text-base`}>
                    {faq.question}
                  </span>
                  <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-transform duration-200 ${
                ...
                {isOpen && (
                  <div className="px-3.5 sm:px-6 pb-3 sm:pb-5 pt-1 text-[11px] sm:text-sm text-slate-600 leading-relaxed border-t border-slate-100">
                    <p>{faq.answer}</p>
                  </div>
                )}
              </div>
        ...
        <div className="mt-4 sm:mt-10 p-3.5 sm:p-6 bg-slate-50 rounded-xl sm:rounded-2xl border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
          <div className="flex items-center gap-2.5 text-left w-full sm:w-auto">
            <div className="w-9 h-9 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-pink-100 text-[#E81E76] flex items-center justify-center flex-shrink-0">
              <HelpCircle className="w-4 h-4 sm:w-6 sm:h-6" />
            </div>
            <div>
              <h4 className="font-bold text-slate-900 text-xs sm:text-sm">Have more questions?</h4>
              <p className="text-[10px] sm:text-xs text-slate-500">Our specialists are ready to assist you.</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
            <a
              ...
              className="px-3 sm:px-4 py-1.5 sm:py-2 bg-white hover:bg-slate-100 border border-slate-200 text-slate-800 font-bold text-[11px] sm:text-xs rounded-lg sm:rounded-xl shadow-2xs flex items-center gap-1"
            >
              <Phone className="w-3 h-3 text-[#E81E76]" />
              <span>+91 8006488006</span>
            </a>
            {onOpenContactModal && (
              <button
                onClick={onOpenContactModal}
                className="px-3 sm:px-4 py-1.5 sm:py-2 bg-[#E81E76] hover:bg-[#c2145e] text-white font-bold text-[11px] sm:text-xs rounded-lg sm:rounded-xl shadow-xs cursor-pointer"
              >
                Contact Us
              </button>
            )}
